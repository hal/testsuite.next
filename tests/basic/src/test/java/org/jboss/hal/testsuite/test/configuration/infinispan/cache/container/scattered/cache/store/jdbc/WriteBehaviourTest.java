...
public class WriteBehaviourTest {
    ...
    public static void init() throws IOException, CommandFailedException {
        ...
        operations.headers(Values.of("allow-resource-service-restart", true))
            .add(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND), Values.of("data-source",
                DATA_SOURCE_BEHIND));
        operations.headers(Values.of("allow-resource-service-restart", true))
            .add(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND_EDIT),
                Values.of("data-source", DATA_SOURCE_BEHIND_EDIT));
        operations.headers(Values.of("allow-resource-service-restart", true))
            .add(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_THROUGH), Values.of("data-source",
                DATA_SOURCE_THROUGH));
        operations.batch(
            new Batch().remove(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND).and(WRITE, THROUGH))
                .add(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND).and(WRITE, BEHIND)));
        operations.batch(
            new Batch().remove(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND_EDIT).and(WRITE, THROUGH))
                .add(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND_EDIT).and(WRITE, BEHIND)));
    }
    ...
    public void switchToWriteBehind() throws Exception {
        ...
        page.getJdbcStoreTab().select(ScatteredCachePage.JDBC_STORE_WRITE_BEHAVIOUR_TAB);
        page.switchBehaviour();
        new ResourceVerifier(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_THROUGH).and(WRITE, BEHIND),
            client).verifyExists();
    }
    ...
    public void switchToWriteThrough() throws Exception {
        ...
        page.getJdbcStoreTab().select(ScatteredCachePage.JDBC_STORE_WRITE_BEHAVIOUR_TAB);
        page.switchBehaviour();
        new ResourceVerifier(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND).and(WRITE, THROUGH),
            client).verifyExists();
    }
    ...
    public void editModificationQueueSize() throws Exception {
        ...
        crudOperations.update(jdbcStoreAddress(CACHE_CONTAINER, SCATTERED_CACHE_WRITE_BEHIND_EDIT).and(WRITE, BEHIND),
            page.getJdbcStoreWriteBehindForm(),
