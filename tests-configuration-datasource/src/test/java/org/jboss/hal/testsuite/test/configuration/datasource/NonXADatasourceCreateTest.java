...
public class NonXADatasourceCreateTest extends AbstractDatasourcePropertiesTest {
    ...
    public static void cleanUpDataSources() throws InterruptedException, TimeoutException, IOException, OperationException {
        operations.removeIfExists(dataSourceAddress(PG_DATASOURCE_NAME));
        operations.removeIfExists(dataSourceAddress(SQLSERVER_DATASOURCE_NAME));
    ...
    public void createPostgreSQLDataSources() throws Exception {
        ...
        wizard.close();
        ModelNodeResult result = operations.readAttribute(dataSourceAddress(PG_DATASOURCE_NAME), DATASOURCE_CLASS);
        assertEquals("Bad value of field " + DATASOURCE_CLASS + " in configuration file!","undefined",operations.readAttribute(dataSourceAddress(PG_DATASOURCE_NAME), DATASOURCE_CLASS).value().asString());
    }
    ...
    public void createSqlserverDataSources() throws Exception {
        ...
        wizard.verifySuccess();
        wizard.close();
        assertEquals("Bad value of field " + DATASOURCE_CLASS + " in configuration file!","undefined",operations.readAttribute(dataSourceAddress(SQLSERVER_DATASOURCE_NAME), DATASOURCE_CLASS).value().asString());
    }
}
