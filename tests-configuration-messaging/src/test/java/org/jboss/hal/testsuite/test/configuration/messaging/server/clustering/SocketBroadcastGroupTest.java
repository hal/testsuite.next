...
public class SocketBroadcastGroupTest extends AbstractClusteringTest {

    private static final String MESSAGING_BROADCAST_GROUP = "messaging-broadcast-group";
    private static final String MESSAGING_SOCKET_BROADCAST_GROUP = "msg-socket-broadcast-group";
    ...
    public static void createResources() throws IOException {
        ...
        operations.add(socketBroadcastGroupAddress(SRV_UPDATE, BG_UPDATE), Values.of(SOCKET_BINDING, HTTP)
                .and(CONNECTORS, CONNECTORS_LIST)).assertSuccess();
        operations.add(socketBroadcastGroupAddress(SRV_UPDATE, BG_DELETE), Values.of(SOCKET_BINDING, HTTP)
                .and(CONNECTORS, CONNECTORS_LIST)).assertSuccess();
    }
    ...
    public void broadcastGroupCreate() throws Exception {
        console.verticalNavigation().selectSecondary(Ids.build(MESSAGING_BROADCAST_GROUP, ITEM), Ids.build(MESSAGING_SOCKET_BROADCAST_GROUP, ITEM));
        TableFragment table = page.getSocketBroadcastGroupTable();
        FormFragment form = page.getSocketBroadcastGroupForm();
        table.bind(form);

        crudOperations.create(socketBroadcastGroupAddress(SRV_UPDATE, BG_CREATE), table, formFragment -> {
    ...
    public void broadcastGroupUpdate() throws Exception {
        ...
        crudOperations.update(socketBroadcastGroupAddress(SRV_UPDATE, BG_UPDATE), form, BROADCAST_PERIOD, 123L);
    }
    ...
    public void broadcastGroupRemove() throws Exception {
        ...
        crudOperations.delete(socketBroadcastGroupAddress(SRV_UPDATE, BG_DELETE), table, BG_DELETE);
    }
